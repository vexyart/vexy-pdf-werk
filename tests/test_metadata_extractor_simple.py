# this_file: tests/test_metadata_extractor_simple.py
"""Simplified tests for metadata extraction functionality."""
...
class TestMetadataExtractor:
    ...
    def test_calculate_word_count(self, extractor, sample_markdown_result):
        ...
        # The method filters out markdown formatting and short words
        # Should count meaningful words from both pages
        assert word_count > 0
        assert word_count < 20  # Should be reasonable
    ...
    def test_get_first_page_preview(self, extractor, sample_markdown_result):
        ...
        # Should extract preview text
        assert preview is not None
        assert isinstance(preview, str)
        assert len(preview) > 0
        assert "introduction" in preview.lower()
    ...
    def test_extract_metadata_basic(self, extractor, sample_pdf_info, sample_markdown_result):
        ...
        with tempfile.NamedTemporaryFile(suffix=".pdf") as temp_file:
            ...
            # Mock file size since the temp file might be empty
            with patch.object(pdf_path, 'stat') as mock_stat:
                ...
                metadata = extractor.extract_metadata(
                    ...
                    formats_generated=["markdown", "epub"],
                    processing_time=8.3
                )

        # Verify basic metadata
        assert metadata.source_file == temp_file.name.split('/')[-1]  # Just filename
        assert metadata.source_size_bytes == 1536000
        ...
        assert metadata.processing_time_seconds == 8.3
    ...
    def test_extract_metadata_minimal(self, extractor):
        ...
        assert metadata.source_file == temp_file.name.split('/')[-1]
        assert metadata.source_size_bytes == 1024
...
class TestMetadataExtractorEdgeCases:
    ...
    def test_calculate_word_count_with_special_characters(self, extractor):
        ...
        # Should count meaningful words, filtering out markdown
        assert word_count > 3  # At least some words counted
        assert word_count < 10  # Reasonable upper bound

    def test_get_first_page_preview_long_content(self, extractor):
        """Test first page preview with long content."""
        long_content = "This is a very long piece of content. " * 10  # 380+ characters
        pages = [
            MarkdownPage(
                page_number=0,
        ...
        assert preview is not None
        assert len(preview) <= 250  # Should be reasonable length
        assert "very long piece" in preview

    def test_extract_and_save_integration(self, extractor):
        """Test complete extract and save workflow."""
        with tempfile.NamedTemporaryFile(suffix=".pdf") as temp_pdf, \
             tempfile.NamedTemporaryFile(suffix=".yaml", delete=False) as temp_yaml:
            ...
            markdown_result = MarkdownResult(success=True, pages=pages, total_pages=1)
            ...
            try:
                with patch.object(pdf_path, 'stat') as mock_stat:
                    ...
                    metadata = extractor.extract_metadata(
                        ...
                        formats_generated=["markdown", "yaml"],
                        processing_time=5.2
                    )

                    # Save to YAML
                    extractor.save_metadata_yaml(metadata, yaml_path)

                # Verify file was created and has content
                assert yaml_path.exists()
                content = yaml_path.read_text(encoding='utf-8')
                assert "pdf_title: Integration Test" in content
                assert "processing_time_seconds: 5.2" in content
                assert "- markdown" in content
